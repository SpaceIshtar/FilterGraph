...
void search_queryfile(std::string& query_path, std::string& query_label_file, std::string& index_prefix, std::string& gt_file, 
    ...
    // #pragma omp parallel for reduction(+:computation_count)
    for (uint32_t i=0;i<nq;i++){
        computation_count+=index->search(query+i*dim,query_to_labels[i],topk,result+i*topk,distance_res+i*topk,ef_search);
    }
